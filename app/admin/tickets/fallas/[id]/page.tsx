...
const statusLabels: Record<string, { label: string; color: string }> = {
    'NEW': { label: 'Recibido', color: 'bg-blue-500' },
    'DIAGNOSIS': { label: 'En Revisión', color: 'bg-yellow-500' },
    'SCHEDULED': { label: 'Agendado', color: 'bg-indigo-500' },
    'IN_PROGRESS': { label: 'En Reparación', color: 'bg-orange-500' },
    'RESOLVED': { label: 'Resuelto', color: 'bg-green-500' },
    'CLOSED': { label: 'Cerrado', color: 'bg-gray-500' },
    'NOT_APPLICABLE': { label: 'No Aplica', color: 'bg-gray-500' },
};

const statusFlow = ['NEW', 'DIAGNOSIS', 'SCHEDULED', 'IN_PROGRESS', 'RESOLVED'];
...
export default function FaultTicketDetailPage() {
    ...
    const handleStatusChange = async () => {
        ...
        try {
            ...
            await supabase.from('ticket_status_history').insert({
                ticket_id: ticket.id,
                previous_status: ticket.fault_status,
    ...
    if (!ticket) {
        return (
            <div className="text-center py-12">
                ...
                <Link href="/admin/tickets/fallas" className="text-primary hover:underline mt-4 inline-block">
                    ← Volver a la lista
                </Link>
            </div>
        );
    ...
    const currentStatusIndex = statusFlow.indexOf(ticket.fault_status);
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{ticket.folio}</h1>
                        <p className="text-gray-500">Ticket de Falla</p>
                    </div>
                </div>
                        ...
                        return (
                            <div key={status} className="flex items-center flex-1">
                                <div className="flex flex-col items-center">
                                    ...
                                    <span className={`text-xs mt-2 ${isCurrent ? 'font-semibold text-gray-900 dark:text-gray-100' : 'text-gray-500'
                                        }`}>
                                        {statusInfo.label}
                                    </span>
                                </div>
                                {index < statusFlow.length - 1 && (
                                    <div className={`flex-1 h-1 mx-2 ${isCompleted ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-600'
                                        }`} />
                                )}
                            </div>
                        );
                ...
                <div className="p-6">
                    ...
                    {activeTab === 'details' && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                <h3 className="font-semibold text-gray-900 dark:text-gray-100">Datos del Cliente</h3>
                                <div className="space-y-4">
                                    ...
                                    {ticket.service_number && (
                                        <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                                            <p className="text-sm text-blue-600 dark:text-blue-400">Número de Servicio</p>
                                            <p className="font-bold text-blue-800 dark:text-blue-200">{ticket.service_number}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <h3 className="font-semibold text-gray-900 dark:text-gray-100">Descripción del Problema</h3>
                                <div className="p-4 bg-red-50 dark:bg-red-900/30 rounded-lg">
                                    <p className="text-red-800 dark:text-red-200 whitespace-pre-wrap">
                                        {ticket.fault_description || 'No se proporcionó descripción'}
                                    </p>
                                </div>
                                    ...
                                    <div key={history.id} className="flex gap-4">
                                        ...
                                        <div className="pb-6">
                                            ...
                                            {history.change_reason && (
                                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{history.change_reason}</p>
                                            )}
                                        ...
                                        <div key={event.id} className={`p-4 rounded-lg ${event.is_visible_to_customer
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div className="flex-1">
                                                    <p className="text-gray-900 dark:text-gray-100">{event.content}</p>
                                                </div>
